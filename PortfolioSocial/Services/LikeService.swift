        ...
        likesRef.setValue(true) { (error, _) in
            ...
            UserService().getUUID(from: post.poster, completion: { (uuid) in
                ...
                likesRef.setValue(true) { (error, _) in
                    ...
                    let likeCountRef = Database.database().reference().child("posts").child(uuid).child(key).child("like_count")
                ...
                likesRef.setValue(nil) { (error, _) in
                    ...
                    likeCountRef.runTransactionBlock({ (mutableData) -> TransactionResult in
                        ...
                        if let error = error {
                            ...
                            ActivityService().deleteEvent(forURLString: key, from: uuid, completion: { (bool) in
                                success(true)
                            })
                        }
                    })
