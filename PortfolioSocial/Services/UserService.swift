...
class UserService {
    func create(_ firUser: FIRUser, username: String, completion: @escaping (User?) -> Void) {
        ...
        ref.setValue(userAttrs) { (error, ref) in
            ...
            Database.database().reference().child("active_usernames").child(username).setValue("true") { (error, ref) in
                if let error = error {
                    assertionFailure(error.localizedDescription)
                    return completion(nil)
                }
            ...
            Database.database().reference().child("UsernameToUUID").child(username).setValue(firUser.uid) { (error, ref) in
                if let error = error {
                    assertionFailure(error.localizedDescription)
                    return completion(nil)
                }
            ...
            ref.child("numberOfPosts").setValue("0") { (error, ref) in
                if let error = error {
                    assertionFailure(error.localizedDescription)
                    return completion(nil)
                }
    ...
    func checkUsername(username: String, completion: @escaping (Bool) -> Void)  {
        Database.database().reference().child("active_usernames").child(username).observeSingleEvent(of: .value, with: {(usernameSnap) in
